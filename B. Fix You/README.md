## [B. Fix You](https://codeforces.com/problemset/problem/1469/B)
...
Every cell, except `(n, m)`, has a direction `R` (Right) or `D` (Down) assigned to it.  
If the cell `(i, j)` is assigned direction `R`, any luggage kept on that will move to the cell `(i, j+1)`.  
...
Please determine the minimum number of cells you have to change.
...
The `j`-th character in the `i`-th line, `aᵢ,ⱼ`, is the initial direction of the cell `(i, j)`.  
Please note that `aₙ,ₘ = C`.
...
You can verify that the resulting belt is functional.  
For example, if we place any luggage at `(2, 2)`, it first moves to `(3, 2)` and then to `(3, 3)`.
